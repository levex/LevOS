...
uint8_t kybrd_ctrl_read_status () {
	...
	return inport (KYBRD_CTRL_STATS_REG);
}
...
void kybrd_ctrl_send_cmd (uint8_t cmd) {
	...
	outport (KYBRD_CTRL_CMD_REG, cmd);
}

// read keyboard encoder buffer
uint8_t kybrd_enc_read_buf () {

	return inport (KYBRD_ENC_INPUT_BUF);
}
...
void kybrd_enc_send_cmd (uint8_t cmd) {
	...
	outport (KYBRD_ENC_CMD_REG, cmd);
}

void _cdecl kkybrd_read_key()
{
	...
	if (kybrd_ctrl_read_status () & KYBRD_CTRL_STATS_MASK_OUT_BUF) {
		...
		if (code == 0xE0 || code == 0xE1)
			...
			if (code & 0x80) {	//test bit 7
				...
				_scancode = code;
				//fifo_write(&FIFO, &_scancode, sizeof(_scancode));
...
void _cdecl i86_kybrd_irq () {
	...
	/*_buffer[_bufferp] = _scancode;
	_bufferp --;
	_buffernum ++;*/
	//if(_scancode != 0 && _scancode != INVALID_SCANCODE) {
	//	char k = 0;
	/*	fifo_read(&FIFO, &k, 1);
...
KEYCODE kkybrd_get_last_key () {
	...
	/*if(_bufferp>0) {
		return ((KEYCODE)_kkybrd_scancode_std [_buffer[_bufferp--]]);
	}
	return KEY_UNKNOWN;*/
	//char r = 0;
	/*fifo_read(&FIFO, &_scancode, sizeof(_scancode));
	char _r = kybrd_enc_read_buf();
	_scancode = _r;*/
	kkybrd_read_key();
	//fifo_write(&FIFO, &_r, sizeof(_r));
	//if(_scancode != 0)DebugPrintf("0x%X", _scancode);
	//fifo_write(&FIFO, _buf, inport(0x60));
