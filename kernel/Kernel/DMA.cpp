...
void dma_mask_channel(uint8_t channel){
	...
	if (channel <= 4)
		outport(DMA0_CHANMASK_REG, (1 << (channel-1)));
	else
		outport(DMA1_CHANMASK_REG, (1 << (channel-5)));
}
...
void dma_unmask_channel (uint8_t channel) {
	...
	if (channel <= 4)
		outport(DMA0_CHANMASK_REG, channel);
	else
		outport(DMA1_CHANMASK_REG, channel);
}
...
void dma_unmask_all (int dma){
	...
	outport(DMA1_UNMASK_ALL_REG, 0xff);
}
...
void dma_reset_flipflop(int dma){
	...
	// it doesnt matter what is written to this register
	outport( (dma==0) ? DMA0_CLEARBYTE_FLIPFLOP_REG : DMA1_CLEARBYTE_FLIPFLOP_REG, 0xff);
}
...
void dma_set_mode (uint8_t channel, uint8_t mode) {
	...
	outport ( (channel < 4) ? (DMA0_MODE_REG) : DMA1_MODE_REG, chan | (mode) );
	dma_unmask_all ( dma );
}
